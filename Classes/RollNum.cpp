...
bool RollNum::init()
{
    ...
    SpriteFrame *pFrame = CCSpriteFrame::createWithTexture(m_pTexture, CCRectMake(0, 0, NUMBERWIDTH, NUMBERHEIGHT));
...
void RollNum::updateNumber(float dt)
{
    ...
    SpriteFrame *pFrame = CCSpriteFrame::createWithTexture(m_pTexture, CCRectMake(0, h, NUMBERWIDTH, NUMBERHEIGHT));
...
bool RollNumGroup::initWithGameLayer(GameLayer *pGameLayer, int nDigit)
{
    ...
    auto pTex = Director::getInstance()->getTextureCache()->addImage("number.png");
    m_pBatchNode = CCSpriteBatchNode::createWithTexture(pTex);
    m_pGameLayer->addChild(m_pBatchNode, 102);
...
void RollNumGroup::setPosition(Point pt)
{
    m_ptPosition = pt;
